...
- Responsibilities:
  ...
  - Launches the required **services** in containers (via Apptainer).  
...
- Characteristics:
  - Already implemented, ready to be containerized.  
  - Expose APIs and generate logs automatically (`stdout` and `stderr` redirected to `.log` files).  
